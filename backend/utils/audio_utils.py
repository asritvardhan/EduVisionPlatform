# ── EduVision — utils/audio_utils.py ────────────────────────────────────────
import librosa
import numpy as np
...
def extract_audio_features(audio_path):
    """
    Matches OptimizedCREMADClassifier.extract_enhanced_features()
    ...
    T = min(mel_db.shape[0], mfcc_all.shape[0],
            spec_centroid.shape[0], spec_rolloff.shape[0], spec_contrast.shape[0])

    features = np.concatenate([
        mel_db[:T], mfcc_all[:T],
        spec_centroid[:T], spec_rolloff[:T], spec_contrast[:T]
    ], axis=1)
